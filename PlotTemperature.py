...
def assemble_temperature_matrix(problems, solutions):
    ...
    for i in range(len(solutions)):
        s = solutions[i]
        n = int( problems[i].A.distance(problems[i].B)/ problems[i].delta_x ) # (n+1) el in the row 
        m = int( problems[i].A.distance(problems[i].D)/ problems[i].delta_y ) # (m+1) el in a column
        # print("n=",n,"m=",m)
    ...
    grid_x_size = int((max_x - min_x) / delta_x) + 1  # +1 to include the last point
    grid_y_size = int((max_y - min_y) / delta_y) + 1  # +1 to include the last point
    ...
    for i, ((x1, y1, x2, y2), temp_matrix) in enumerate(zip(room_coords, temp_matrices)):
        ...
        start_x = int((x1 - min_x) / delta_x)
        end_x = int((x2 - min_x) / delta_x)
        start_y = int((y1 - min_y) / delta_y)
        end_y = int((y2 - min_y) / delta_y)
