...
def calculate_Dirichlet_cond(v: np.array, 
    ...
    #  i_end = bound_info[2]
     ...
     # sizes of the grid
     n = round( problem.A.distance(problem.B)/ problem.delta_x ) # (n+1) el in the row 
     m = round( problem.A.distance(problem.D)/ problem.delta_y ) # (m+1) el in a column
...
def calculate_Neumann_cond(v: np.array, 
    ...
    # Left boundary
         ...
         raise ValueError("Wrong boundary index, should be intenger in range(0,4)")   
