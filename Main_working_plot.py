...
for i in range(ITERATIONS):    
    ...
    if rank == 0:
        ...
        v_0 = method.solve(problem2)

        if(i!=0): # do relaxation step
            v_0 = v_0*omega + (1-omega)*v_prev_0

        new_types_1, new_values_1 = bc.calculate_new_condition(v_0, problem2, neighbour2_1)
        new_types_3, new_values_3 = bc.calculate_new_condition(v_0, problem2, neighbour2_3)
        new_types_4, new_values_4 = bc.calculate_new_condition(v_0, problem2, neighbour2_4)
    ...
    # ---------------------------- Process 2-------------------------------------------
        ...
        v_1 = method.solve(problem1)

        if(i!=0): # do relaxation step
            v_1 = v_1*omega + (1-omega)*v_prev_1

        new_types, new_values = bc.calculate_new_condition(v_1, problem1, neighbour1)
    ...
    # ---------------------------- Process 3-------------------------------------------
        ...
        v_2 = method.solve(problem3)

        if(i!=0): # do relaxation step
            v_2 = v_2*omega + (1-omega)*v_prev_2

        new_types, new_values = bc.calculate_new_condition(v_2, problem3, neighbour3)
    ...
    # ---------------------------- Process 4-------------------------------------------
        ...
        v_3 = method.solve(problem4)

        if(i!=0): # do relaxation step
            v_3 = v_3*omega + (1-omega)*v_prev_3

        new_types, new_values = bc.calculate_new_condition(v_3, problem4, neighbour4)
